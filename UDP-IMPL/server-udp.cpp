...
int main(){
	...
	while(1)
	//w_socket = accept(sock_fd, (struct sockaddr *)&client, (socklen_t*)&c)) )
    	{
		...
		int len=0;
		int bytes_received = recvfrom(sock_fd,&message[0],2048,0,(struct sockaddr *)&client,(socklen_t*)&addr_len);
	...
	try{
		...
		//write(new_socket,out.c_str(),out.length());         
    		sendto(sock_fd,out.c_str(),out.length(),0,(struct sockaddr *)&client,(socklen_t)sizeof(struct sockaddr));
	//	fflush(sock_fd);
	}
